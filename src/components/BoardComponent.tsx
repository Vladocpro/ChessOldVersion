...
const BoardComponent: FC<BoardProps> = ({board: Board, setBoard, currentPlayer, swapPlayer, lostBlackFigures, lostWhiteFigures, playersNames}) => {
    ...
    let figureAttackingKing :Figure | null = null;
     function click(cell : Cell) {
        ...
        //   movingPiece
        if(selectedCell?.figure && selectedCell !== cell) {
    ...
    function checkCondition(cell : Cell) {
         ...
         // let noOneAttackingKing = false;
         // reset blockingCells
        for (let i = 0; i < 8; i++) {
            for (let j = 0; j < 8; j++) {
                cells[i][j].setBlockingKing(false);
        ...
        function checkPawn(target : Cell) {
            if (target?.figure?.color == Colors.BLACK) {
                if (target.x !== 7) {
                    Board.getCell(target.x + 1, target.y + 1).setBlockingKing(true);
                }
                if (target.x !== 0) {
                    Board.getCell(target.x - 1, target.y + 1).setBlockingKing(true);
                }
        ...
        function checkAndBlock(figure : Figure) {
                if(figure) {
                    for (let i = 0; i < figure.cellsToMove.length; i++) {
                        if(figure.cellsToMove[i].figure?.name === FigureNames.King) {
                            // setFigureAttackingKing(figure);
                            figureAttackingKing=figure;
                            // noOneAttackingKing = true;
                        }
                        figure.cellsToMove[i].setBlockingKing(true);
                    }
                }
        }
    ...
    function gameStatus()  {
         ...
         if(whiteKing.figure?.cellsToMove.length === 0 && whiteKing.blockingKing === true) {
             obj.bool = false;
             obj.color = Colors.BLACK;
             // return obj;
         }
         else  if(blackKing.figure?.cellsToMove.length === 0 && blackKing.blockingKing === true) {
             obj.bool = false;
